...
class CatalogTest extends Model {
	...
	protected $table = 'catalogs_tests';
	protected $_parents = [];
    ...
    public static function getCatalogTestList($parent_id = 0, $lvl = 0) {
        $result = [];
        foreach (self::whereParentId($parent_id)->orderBy('order')->get() as $item) {
            $result[$item->id] = str_repeat('&nbsp;', $lvl * 3) . $item->name;
            $result = $result + self::getCatalogTestList($item->id, $lvl + 1);
        }
	...
	public function parent(): BelongsTo {
		return $this->belongsTo('Fanky\Admin\Models\CatalogTest', 'parent_id');
	}
	...
	public function children(): HasMany {
		return $this->hasMany('Fanky\Admin\Models\CatalogTest', 'parent_id');
	}
    ...
    public function params(): HasMany {
        return $this->hasMany('Fanky\Admin\Models\CatalogTestParam', 'catalog_id')
            ->join('params', 'catalog_params.param_id', '=', 'params.id');
    }

    public function filters(): HasMany {
        return $this->hasMany('Fanky\Admin\Models\CatalogTestFilter', 'catalog_id')
            ->join('params', 'catalog_filters.param_id', '=', 'params.id');
    }

    public function add_params(): HasMany {
        return $this->hasMany(CatalogTestParam::class, 'catalog_id')
            ->join('params', 'catalog_params.param_id', '=', 'params.id');
    }
	...
	public static function getCatalogTests() {
		$catalogs = Cache::get('catalogs', []);
		if (!$catalogs) {
			$catalog_arr = CatalogTest::all(['id', 'parent_id', 'name', 'alias', 'published']);
    ...
    public function findRootCategory($catalog_id) {
        $cur_cat = CatalogTest::find($catalog_id);
	...
	public static function getByPath($path): ?CatalogTest {
		if(is_array($path)){
			$path = implode('/', $path);
		}
    ...
    public function getMainCatalogTest() {
...
//		return $count;
	...
	public static function getRecurseCategory($parent_id) {
		$categories = CatalogTest::whereParentId($parent_id)->pluck('id')->all();
		if (!count($categories))
			return [];
		$result = $categories;
